...
export default function HeroScene3D() {
  ...
  return (
    <div
      ...
      style={{
        background:
          'radial-gradient(60% 55% at 50% 45%, rgba(32,70,255,0.12) 0%, rgba(32,70,255,0) 70%)',
      }}
      aria-hidden="true"
    >
      <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0.2, 5], fov: 35 }} gl={{ antialias: true, alpha: true }}>
        ...
        <directionalLight position={[3, -2, 2]} intensity={0.2} color="#BAC8FF" />

        <CredentialCard />
        <Satellite position={[2.1, 0.9, 0.4]} color="#FF6B4A" shape="cube" delay={0} />
        <Satellite position={[-2.2, -0.6, 0.6]} color="#10B981" shape="torus" delay={0.6} />
        <Satellite position={[1.6, -1.1, -0.3]} color="#F59E0B" shape="sphere" delay={1.2} />
      </Canvas>
    </div>
  );
}
  ...
  return (
    <group ref={groupRef} rotation={[-0.05, 0.4, 0]}>
      ...
      <mesh castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#2046FF" roughness={0.35} metalness={0.15} />
      </mesh>

      {/* Front face overlay — slightly inset, with a subtler gradient via vertex colors */}
      <mesh position={[0, 0, 0.031]}>
        <planeGeometry args={[2.56, 1.61]} />
        <meshStandardMaterial color="#1735C7" roughness={0.55} metalness={0.05} />
      </mesh>

      {/* Brand mark — a simple white plate, kept abstract */}
      ...
      <mesh position={[-1.05, 0.5, 0.041]}>
        ...
        <meshStandardMaterial color="#2046FF" />
      </mesh>
      ...
      {fieldRows.map((row, i) => (
        <mesh key={i} position={[-0.35 + row.width / 2 - 0.85, row.y, 0.04]}>
          ...
          <meshStandardMaterial color="#7E92FF" transparent opacity={0.55} />
        </mesh>
      ))}
      ...
      <mesh position={[0.95, 0.45, 0.04]}>
        ...
        <meshStandardMaterial color="#DDE3FF" roughness={0.4} />
      </mesh>
      <mesh position={[0.95, 0.45, 0.041]}>
        <planeGeometry args={[0.22, 0.02]} />
        <meshStandardMaterial color="#5570FF" />
      </mesh>
      ...
      <mesh position={[0, 0.81, 0]}>
        ...
        <meshStandardMaterial color="#BAC8FF" roughness={0.3} />
      </mesh>
    </group>
  );
}
