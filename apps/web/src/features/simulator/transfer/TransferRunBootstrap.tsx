...
const LAST_RUN_KEY = 'vitality.lastTransferRunId';

export function TransferRunBootstrap() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const run = await simApi.startRun('transfer', locale);
        if (cancelled) return;
        try {
        ...
        navigate(`/simulator/transfer/${run.id}`, { replace: true });
      } catch (err) {
        if (cancelled) return;
        const message =
