...
const LAST_RUN_KEY = 'vitality.lastOpenAccountRunId';

/**
 * Visiting `/simulator/open-account` (without a runId) creates a fresh
 * Open Account run and redirects to `/simulator/open-account/:runId`.
 * Mirror of KycRunBootstrap, but scoped to the open-account scenario.
 */
export function OpenAccountRunBootstrap() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const run = await simApi.startRun('open-account', locale);
        if (cancelled) return;
        try {
        ...
        navigate(`/simulator/open-account/${run.id}`, { replace: true });
      } catch (err) {
        if (cancelled) return;
        const message =
