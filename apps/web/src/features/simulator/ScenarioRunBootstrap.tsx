...
interface Props {
  ...
  /** URL segment, e.g. `open-account` for `/simulator/open-account/:runId`. */
  pathSegment: string;
  lastRunKey: string;
}

/**
 * Visiting `/simulator/:path` (without runId) creates a fresh run and redirects
 * to `/simulator/:path/:runId`.
 */
export function ScenarioRunBootstrap({ scenarioId, pathSegment, lastRunKey }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        try {
          window.localStorage.setItem(lastRunKey, run.id);
        } catch {
          // ignore storage failure
        }
        navigate(`/simulator/${pathSegment}/${run.id}`, { replace: true });
      } catch (err) {
        if (cancelled) return;
        const message =
  ...
  }, [scenarioId, lastRunKey, locale, navigate, pathSegment, t]);
      ...
      >
        {!error ? (
          <div className="space-y-3">
            <LoadingBar />
            <p className="font-mono-tech text-[12px] text-zinc-600">{t('sim.run.loading_run')}</p>
          </div>
        ) : (
